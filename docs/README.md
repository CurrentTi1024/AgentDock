...
| [design/13-concurrent-run-guard.md](agentdock/design/13-concurrent-run-guard.md) | 并发 run 历史问题与当前防护：同 Session 防重入、runtime 幂等守卫、eventId 安全 resume、流中断兜底 |
...
| [design/17-chat-full-feature-architecture.md](agentdock/design/17-chat-full-feature-architecture.md) | Chat 全功能架构：运行时、事件投影、历史、消息与过程块渲染、输入和一致性机制 |
| [design/18-multi-session-concurrent-runs.md](agentdock/design/18-multi-session-concurrent-runs.md) | 多 Session 独立订阅与并发运行：全局 Operation、常驻 Runtime Worker、事件隔离、per-run checkpoint 与未来续传 |
