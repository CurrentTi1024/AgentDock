...
test('parses chunked CRLF SSE frames and preserves stream ids', async () => {
  ...
  assert.deepEqual(events.map((item) => [item.streamId, item.event.type]), [['1', 'RUN_STARTED'], ['2', 'RUN_FINISHED']]);
});
