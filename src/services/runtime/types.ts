...
export interface RuntimeRunState { activities: Record<string, unknown>; error?: { code?: string; message: string }; latestStreamId?: string; messages: Record<string, RuntimeMessage>; processedStreamIds: string[]; rawEvents: AgUiEvent[]; reasoning: Record<string, string>; runId: string; state: unknown; status: RunStatus; surfaces: Record<string, unknown>; threadId: string; toolCalls: Record<string, { args: string; name?: string; result?: unknown; status: string }> }
