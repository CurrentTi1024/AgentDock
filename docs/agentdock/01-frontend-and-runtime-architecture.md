...
### 5.5 多 Session 运行所有权（当前实现）

多 Session 并发不能由 ChatPage 局部 Hook 持有 Agent 订阅。当前运行时采用：
...
- 页面刷新恢复：会话列表与当前会话消息从 IndexedDB 读取；running checkpoint 有顶层 `latestEventId` 时，以相同 `runId` 请求游标后的增量事件，没有游标时才转为 cancelled，禁止整轮重放。
