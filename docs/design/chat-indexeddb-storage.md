...
- **sequence**：会话内单调递增的排序号（时间戳 ×1000 + 0..999 种子）。已存在行落库时**保留原 sequence**，这是“多轮时间线不乱”的基石。
...
4. 游标 = 页内最旧文本的 sequence（严格更早）；`hasMore` 用索引探测游标之前是否还有文本。
...
- 事件协议：`agentdock:storage-warning`（AppShell 弹提示，设置页进度条 + 健康状态）。
