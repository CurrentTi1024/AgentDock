...
- **sequence**：会话内单调递增的排序号（时间戳 ×1000 + 递增种子，种子随机起步、不取模；`lastSequence` 兜底——即使系统时钟回拨也强制新行大于已分配最大值，从结构上保证“后落库的消息一定排在后”，错误消息因此恒为该轮最后一条；跨标签页同毫秒并发写同一会话的碰撞概率压到约 1/1000）。已存在行落库时**保留原 sequence**，新行按 messageOrder 出现顺序分配（后一轮自动 +1、严格更大），这是“多轮时间线不乱”的基石。
...
**占位+增量复核（对齐 LobeHub 后）**

- 墓碑上限：`deletedMessageIds` 防复活数组上限 1000 条（淘汰最旧），避免长会话频繁编辑/重新生成导致无限膨胀；新增上限测试。
- 空占位显示防护：历史渲染跳过“无内容且无过程块”的 assistant 占位行（START 后即停止/刷新产生），避免空气泡；有工具/推理块的空气泡保留（块需要 assistant 文本作宿主）。
- 复核确认：删除事务已包含 sessions（无 Dexie 事务作用域错误）；墓碑 key 与 persistRunSnapshot 的 `kind:rawId` 一致；占位行保持文本先于过程块的顺序，removeMessages 整块删除不失效。
