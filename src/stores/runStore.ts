...
export const useRunStore = create<RunStore>((set, get) => ({
  ...
  async resume() { const { activeInput, run } = get(); if (!activeInput || !run?.latestStreamId) return; await get().execute(createRuntimeAction(activeInput, 'resume', { resume: { lastStreamId: run.latestStreamId } })); },
