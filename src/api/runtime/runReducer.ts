...
export const createRunState = (runId: string, threadId: string): RuntimeRunState => ({ activities: {}, messages: {}, processedStreamIds: [], rawEvents: [], reasoning: {}, runId, state: {}, status: 'idle', steps: {}, surfaces: {}, threadId, toolCalls: {} });
...
export function reduceRunEvent(previous: RuntimeRunState, input: StreamedEvent): RuntimeRunState {
  ...
  switch (event.type) {
    ...
    case 'STEP_STARTED': {
      ...
      next.steps[stepId] = { id: stepId, name: String(event.stepName || ''), status: 'running', startedAt: Date.now() };
      break;
    }
    case 'STEP_FINISHED': {
