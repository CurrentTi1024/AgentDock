...
| 11 | 断线重连（eventId） | 首轮 run 后按 lastEventId resume | ✅ 服务层：69 事件全带 eventId；resume 第 40 条精确回放 29 条、无模型调用；未知 runId→STREAM_EXPIRED。⚠️ runtime single-route 纯尾回放受 SSE 校验限制（须 RUN_STARTED 开头），真实接入用 agent/connect 或全量回放+去重 |
